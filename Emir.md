...
// Rastgele gecikme süresi için bir fonksiyon oluşturuyoruz
      ...
      try {
        ...
        while (counter > 0) {
          try {
            ...
            if (counter > 0) {
              const randomDelay = getRandomDelay(30000, 60000); // 30 ile 60 saniye arasında rastgele gecikme
              console.log(
                `⏳ Waiting for ${randomDelay / 1000} seconds before the next transaction...`.yellow
              );
